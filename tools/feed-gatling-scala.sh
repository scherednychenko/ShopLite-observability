#!/usr/bin/env bash
# Run the ShopLite Gatling (Scala) simulation against this stack's mock, then
...
GATLING_REPO="${GATLING_REPO:-$ROOT/../ShopLite-load-tests-gatling-scala}"
NET="${NET:-shoplite-observability_default}"
VUS="${VUS:-10}"; CART_SIZE="${CART_SIZE:-8}"; SIM="${SIM:-ShopLiteGatlingScala}"
IMG="shoplite-gatling-scala"

cd "$ROOT"

[ -f "$GATLING_REPO/gatling/simulations/ShopLiteSimulation.scala" ] || {
  echo "x gatling scala repo not found at ${GATLING_REPO} - set GATLING_REPO=/path/to/ShopLite-load-tests-gatling-scala"; exit 1; }
...
echo "-> building the Gatling (Scala) image..."
docker build -t "$IMG" -f "$GATLING_REPO/gatling/Dockerfile" "$GATLING_REPO" >/dev/null

RES="$(mktemp -d)"
echo "-> running Gatling (Scala) -> ${RES}..."
docker run --rm --network "$NET" \
  -e BASE_URL=http://mock:8080 -e CART_SIZE="$CART_SIZE" -e VUS="$VUS" \
  -v "$RES":/results "$IMG" \
  -rm local -s ShopLiteSimulation -rf /results -rd "ShopLite Gatling (Scala) demo" \
